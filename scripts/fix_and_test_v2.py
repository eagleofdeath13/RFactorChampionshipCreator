"""
Complete fix: patch ALL issues and recreate championship
"""
import sys
import shutil
...
# =======================
# PATCH 2: Fix create_championship to pass isolated_paths instead of len
# =======================
original_create_championship = ChampionshipCreator.create_championship

def patched_create_championship(self, championship_name, vehicle_assignments, tracks, options=None):
    """Patched version that passes isolated_paths to _create_rfm."""
    options = options or {}
    ...
    try:
        # PATCH: Pass isolated_paths instead of len(isolated_paths)
        rfm = self._create_rfm(
            championship_name,
            tracks,
            isolated_paths,  # <-- FIXED: Pass paths, not count
            options
        )
    except Exception as e:
...
ChampionshipCreator.create_championship = patched_create_championship
print("[PATCH 2] Applied: create_championship now passes isolated_paths")

# =======================
# PATCH 3: Fix _create_rfm to accept isolated_paths and extract vehicle names
# =======================
def patched_create_rfm(self, championship_name, tracks, isolated_paths, options):
    ...
    print(f"[PATCH 3] Extracted {len(starting_vehicles)} vehicle names:")
...
print("[PATCH 3] Applied: _create_rfm now extracts vehicle names for StartingVehicle")
