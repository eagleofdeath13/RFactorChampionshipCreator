...
export default function TalentEdit() {
  ...
  useEffect(() => {
    if (talent) {
      setFormData({
        ...
        starts: talent.personal_info?.starts || 0,
        poles: talent.personal_info?.poles || 0,
        wins: talent.personal_info?.wins || 0,
        drivers_championships: talent.personal_info?.drivers_championships || 0,
        aggression: talent.stats?.aggression || 50.0,
        reputation: talent.stats?.reputation || 50.0,
        courtesy: talent.stats?.courtesy || 50.0,
        composure: talent.stats?.composure || 50.0,
        speed: talent.stats?.speed || 50.0,
        crash: talent.stats?.crash || 50.0,
        recovery: talent.stats?.recovery || 50.0,
        completed_laps: talent.stats?.completed_laps || 90.0,
        min_racing_skill: talent.stats?.min_racing_skill || 50.0,
      })
    }
  }, [talent])
  ...
  const updateMutation = useMutation({
    mutationFn: async (data) => {
      const response = await apiEndpoints.talents.update(name, {
        personal_info: {
    ...
    onSuccess: (data) => {
      ...
      queryClient.invalidateQueries(['talent', name])
    ...
    onError: (error) => {
      if (error.response?.data?.detail) {
        ...
        setErrors({ submit: 'Erreur lors de la modification du talent' })
      }
    },
  })
    ...
    return (
      <div>
        ...
        <RacingCard className="p-6 border-status-danger">
          <p className="text-status-danger font-bold">Talent introuvable</p>
        </RacingCard>
      </div>
    )
  }
  ...
  return (
    <div>
      <PageHeader
        icon={Edit}
        title={`Modifier ${talent.name}`}
        subtitle="Mettre à jour les informations du pilote"
      />
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="flex gap-4">
            ...
            <RacingButton
              type="button"
              variant="secondary"
              onClick={() => navigate(`/talents/${encodeURIComponent(name)}`)}
            >
