Alabama500
{
  ...
  Attrition = 30
  TrackName = Alabama Superspeedway
  EventName = Alabama 500
  GrandPrixName = Alabama 500			//this must be the same as event name in order to sort circuit info correctly.
  VenueName = Alabama Superspeedway
  Location = Talladega County, AL, USA
  Length = 4.02 km / 2.50 miles
  TrackType = Superspeedway Oval
  ...
  RollingStartLoc=1
  GarageDepth = 2.00
  TerrainDataFile = ..\BamaSuperspeedway.tdf // terrain file override
  ...
  Latitude = 33.265          // degs from Equator (range: -90 to 90, positive is Northern Hemisphere)
...
ScoreboardFont=alabama_scorefont.tga // default is scoreboardfont.bmp
ScoreboardBackground=alabama_scorebg.tga // default is scoreboardbkg.bmp

ScoreboardMaxEntries=10 // how many car numbers can be displayed on tower (default is 32)
ScoreboardStartX=-.07 // x-position in texture to write first car number (default is 0)
ScoreboardStartY=-2.0 // y-position in texture to write first car number (default is 10)
ScoreboardIncX=0 // increment in x-position for each new car number (default is 0)
ScoreboardIncY=51 // increment in y-position for each new car number (default is 16)
ScoreboardScaleX=2.2 // scale multiplier for x (default is 1.0)
ScoreboardScaleY=2.25 // scale multiplier for y (default is 1.0)

//////////////////////////////////////////////////////////////////////////////////////

  
  SettingsFolder = Alabama
